...
export default function CsrDashboard() {
  ...
  async function downloadCertificate() {
    ...
    // ── TOP-LEFT: EcoTree Logo block (vector text, no image)
    doc.setFillColor(26, 60, 52)
    doc.roundedRect(10, 8, 58, 20, 2, 2, 'F')
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(11)
    doc.setTextColor(255, 255, 255)
    doc.text('ECO TREE', 39, 15, { align: 'center' })
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(6)
    doc.setTextColor(151, 188, 98)
    doc.text('IMPACT FOUNDATION', 39, 20, { align: 'center' })
    doc.setFontSize(5)
    doc.setTextColor(200, 220, 180)
    doc.text('PLANT  |  PROTECT  |  PRESERVE', 39, 25, { align: 'center' })

    // ── CERT ID + DATE — center top
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(9)
    doc.setTextColor(60, 60, 60)
    doc.text('Certificate ID: ', W/2 - 28, 14)
    doc.setFont('helvetica', 'bold')
    doc.setTextColor(26, 60, 52)
    doc.text(certId, W/2, 14)
    doc.setFont('helvetica', 'normal')
    doc.setTextColor(60, 60, 60)
    doc.text('Date of Issue: ', W/2 - 24, 21)
    doc.setFont('helvetica', 'bold')
    doc.setTextColor(26, 60, 52)
    doc.text(issueDate, W/2 + 4, 21)
    doc.setFont('helvetica', 'normal')

    // ── COMPANY NAME BOX — top right
    doc.setDrawColor(200, 168, 76)
    doc.setLineWidth(0.8)
    doc.roundedRect(W - 58, 8, 46, 20, 2, 2, 'D')
    doc.setFontSize(9)
    doc.setFont('helvetica', 'bold')
    doc.setTextColor(26, 60, 52)
    const shortName = partner.company_name.length > 16
      ? partner.company_name.substring(0, 14) + '..'
      : partner.company_name
    doc.text(shortName.toUpperCase(), W - 35, 16, { align: 'center' })
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(6)
    doc.setTextColor(100, 140, 100)
    doc.text('CSR PARTNER', W - 35, 22, { align: 'center' })
    ...
    doc.setFontSize(20)
    doc.setTextColor(26, 60, 52)
    doc.text('VERIFIED ENVIRONMENTAL IMPACT CERTIFICATE', W/2, 44, { align: 'center' })
    ...
    doc.text('Presented To', W/2, 58, { align: 'center' })

    // Company name — large bold
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(26)
    doc.setTextColor(26, 60, 52)
    doc.text(partner.company_name.toUpperCase(), W/2, 70, { align: 'center' })
    doc.setFont('helvetica', 'normal')
    ...
    doc.line(40, 74, W - 40, 74)
    doc.setLineDashPattern([], 0)

    // ── RECOGNITION TEXT — bold black
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(11)
    doc.setTextColor(26, 26, 26)
    doc.text('In recognition of your commitment towards environmental sustainability,', W/2, 83, { align: 'center' })
    doc.text('urban greening and climate action through support for our', W/2, 90, { align: 'center' })
    doc.setTextColor(44, 95, 45)
    doc.text('Tree Plantation & Ecological Restoration Initiative.', W/2, 97, { align: 'center' })
    doc.setFont('helvetica', 'normal')

    // ── 3 STAT BOXES
    const stats = [
      { val: String(partner.trees_planted || 0), unit: '',         label:'TREES PLANTED',    x: 20  },
      { val: String(co2Val),                      unit: 'KG/YR',   label:'CO2 OFFSET',       x: 100 },
      { val: String(Math.round(o2Val / 1000)),    unit: 'TONS/YR', label:'OXYGEN GENERATED', x: 180 },
    ]

    doc.setFillColor(248, 252, 248)
    doc.setDrawColor(180, 210, 160)
    doc.setLineWidth(0.5)
    doc.roundedRect(16, 103, 200, 32, 3, 3, 'FD')

    stats.forEach((s, i) => {
      if (i > 0) {
        doc.setDrawColor(180, 210, 160)
        doc.setLineWidth(0.4)
        doc.line(s.x - 10, 107, s.x - 10, 131)
      }
      // Filled circle icon
      doc.setFillColor(44, 95, 45)
      doc.circle(s.x + 6, 119, 9, 'F')
      // Large stat number
      doc.setFont('helvetica', 'bold')
      doc.setFontSize(20)
      doc.setTextColor(26, 60, 52)
      doc.text(s.val, s.x + 20, 118, { align: 'left' })
      // Unit — smaller, to the right of number
      if (s.unit) {
        const numW = doc.getTextWidth(s.val)
        doc.setFont('helvetica', 'normal')
        doc.setFontSize(6.5)
        doc.setTextColor(80, 80, 80)
        doc.text(s.unit, s.x + 20 + numW + 1, 113)
      }
      // Label
      doc.setFont('helvetica', 'bold')
      doc.setFontSize(6.5)
      doc.setTextColor(100, 140, 100)
      doc.text(s.label, s.x + 20, 125, { align: 'left' })
      doc.setFont('helvetica', 'normal')
    })
    ...
    doc.line(16, 139, 216, 139)

    // ── PROJECT DETAILS
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(8.5)
    doc.setTextColor(26, 60, 52)
    doc.text('PROJECT DETAILS', 116, 146, { align: 'center' })
    doc.setFont('helvetica', 'normal')
    doc.setDrawColor(200, 168, 76)
    doc.setLineWidth(0.4)
    doc.line(16, 149, 216, 149)

    const leftDetails = [
      { label: 'Project Name',    val: partner.project_type || 'Tree Plantation' },
    ...
    rightDetails.forEach((d, i) => {
      ...
      doc.setFont('helvetica', 'bold')
      doc.setTextColor(44, 95, 45)
      doc.text(d.label, 120, y)
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(40, 40, 40)
      doc.text(': ' + d.val, 158, y)
    })

    // ── QR CODE — bottom right (inside project details area)
    const qrDataUrl = await QRCode.toDataURL('https://ecotrees.org/csr-dashboard', {
      width: 300, margin: 2,
      color: { dark: '#1A3C34', light: '#FFFFFF' },
    })
    doc.addImage(qrDataUrl, 'PNG', W - 48, 148, 32, 32)
    doc.setFontSize(5.5)
    doc.setTextColor(80, 80, 80)
    doc.text('View live dashboard', W - 32, 183, { align: 'center' })
    ...
    doc.setFontSize(8.5)
    doc.setTextColor(255, 255, 255)
    doc.text('"The best time to plant a tree was 20 years ago.', 12, H - 17)
    doc.setTextColor(151, 188, 98)
    doc.text('The second best time is now."', 12, H - 10)
    doc.setFont('helvetica', 'normal')

    // 80G badge — center
    doc.setFillColor(44, 95, 45)
    doc.roundedRect(W/2 - 48, H - 22, 96, 11, 2, 2, 'F')
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(6)
    doc.setTextColor(151, 188, 98)
    ...
    doc.setFontSize(13)
    doc.setTextColor(255, 255, 255)
    doc.text('Bhimsen G', W - 18, H - 17, { align: 'right' })
    doc.setDrawColor(151, 188, 98)
    doc.setLineWidth(0.5)
    doc.line(W - 70, H - 13, W - 12, H - 13)
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(6.5)
    doc.setTextColor(151, 188, 98)
    doc.text('Founder & Director  |  Eco Tree Impact Foundation', W - 18, H - 8, { align: 'right' })
