...
interface PhotoPopup {
  url: string; label: string; treeId: string
  itemId: number; health: number | null; lat: number | null; lng: number | null
}
...
export default function AdminReview() {
  ...
  async function handleCsrApprove(batch: CsrBatchReview) {
    ...
    if (!error && batch.contact_email) {
      await fetch('/api/create-donor', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: batch.contact_email, password: '123456', name: batch.contact_name || batch.company_name, donorId: 0 }),
      })
      await sendEmail('csr_verified', {
        name: batch.contact_name || batch.company_name, email: batch.contact_email,
        company: batch.company_name, trees_planted: batch.trees_planted, tree_count: batch.tree_count,
        site: batch.sites?.name || 'Bangalore', project_type: batch.project_type || 'Tree Plantation',
  ...
  function CheckBadge({ pass, label }: { pass: boolean | null; label: string }) {
    ...
    return <div style={{ background:bg, borderRadius:6, padding:'4px 6px', textAlign:'center', fontSize:'11px', color:text, fontWeight:500 }}>{icon} {label}</div>
  }

  const totalQueue = items.length + csrBatches.length + miyawakis.length

  return (
    <div>
      <div style={{ marginBottom:'1.25rem' }}>
        <h1 style={{ fontSize:'1.25rem', fontWeight:600, color:'#1A1A1A', marginBottom:'4px' }}>Review Queue</h1>
        <p style={{ fontSize:'14px', color:'#6B7280', display:'flex', gap:'8px', flexWrap:'wrap', alignItems:'center' }}>
          {totalQueue} item{totalQueue!==1?'s':''} waiting
          {csrBatches.length > 0 && <span style={{ background:'#dbeafe', color:'#1e40af', fontSize:'12px', padding:'2px 8px', borderRadius:'10px', fontWeight:600 }}>🏢 {csrBatches.length} CSR</span>}
          {miyawakis.length  > 0 && <span style={{ background:'#f5f3ff', color:'#7C3AED', fontSize:'12px', padding:'2px 8px', borderRadius:'10px', fontWeight:600 }}>🏙️ {miyawakis.length} Miyawaki</span>}
          {items.length      > 0 && <span style={{ background:'#dcfce7', color:'#166534', fontSize:'12px', padding:'2px 8px', borderRadius:'10px', fontWeight:600 }}>🌱 {items.length} Trees</span>}
        </p>
      </div>
      ...
      {loading ? (
        ...
        <>
                    ...
                    <div key={batch.id} style={{ background:'white', border:'1.5px solid #bfdbfe', borderRadius:'12px', overflow:'hidden' }}>
                      <div style={{ padding:'10px 14px', background:'#eff6ff', borderBottom:'1px solid #bfdbfe', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'8px' }}>
                        <div>
                          ...
                          <div style={{ fontSize:'12px', color:'#3b82f6' }}>👷 {batch.worker?.name||'—'} · 📍 {batch.sites?.name||'—'} · {batch.project_type||'Tree Plantation'}</div>
                        </div>
                        <div style={{ display:'flex', gap:'6px' }}>
                          <button onClick={() => handleCsrApprove(batch)} disabled={processing}
                            style={{ padding:'6px 16px', background:'#2C5F2D', color:'white', border:'none', borderRadius:'8px', fontSize:'13px', fontWeight:600, cursor:'pointer' }}>✓ Approve</button>
                          <button onClick={() => handleCsrReject(batch.id)} disabled={processing}
                            style={{ padding:'6px 12px', background:'transparent', color:'#dc2626', border:'1px solid #fecaca', borderRadius:'8px', fontSize:'13px', fontWeight:600, cursor:'pointer' }}>✕ Reject</button>
                        </div>
                      </div>
                      <div style={{ padding:'12px 14px' }}>
                        <div style={{ marginBottom:'12px' }}>
                        ...
                        {(beforePhotos.length > 0 || afterPhotos.length > 0 || sitePhotos.length > 0) && (
                          <div style={{ marginBottom:'10px' }}>
                            {beforePhotos.length > 0 && (
                              <div style={{ marginBottom:'8px' }}>
                                <div style={{ fontSize:'11px', fontWeight:600, color:'#6B7280', textTransform:'uppercase', marginBottom:'4px' }}>Before ({beforePhotos.length})</div>
                                <div style={{ display:'flex', gap:'6px', flexWrap:'wrap' }}>
                                  {beforePhotos.map((url,i) => <div key={i} onClick={() => setBatchPhoto({ url, label:`Before · ${batch.company_name}` })} style={{ width:'72px', height:'72px', borderRadius:'8px', background:`url(${url}) center/cover`, cursor:'pointer', border:'1px solid #e5e7eb' }} />)}
                                </div>
                              </div>
                            )}
                            {afterPhotos.length > 0 && (
                              <div style={{ marginBottom:'8px' }}>
                                <div style={{ fontSize:'11px', fontWeight:600, color:'#166534', textTransform:'uppercase', marginBottom:'4px' }}>After ({afterPhotos.length})</div>
                                <div style={{ display:'flex', gap:'6px', flexWrap:'wrap' }}>
                                  {afterPhotos.map((url,i) => <div key={i} onClick={() => setBatchPhoto({ url, label:`After · ${batch.company_name}` })} style={{ width:'72px', height:'72px', borderRadius:'8px', background:`url(${url}) center/cover`, cursor:'pointer', border:'1px solid #86efac' }} />)}
                                </div>
                              </div>
                            )}
                            {sitePhotos.length > 0 && (
                              <div>
                                <div style={{ fontSize:'11px', fontWeight:600, color:'#6B7280', textTransform:'uppercase', marginBottom:'4px' }}>Site ({sitePhotos.length})</div>
                                <div style={{ display:'flex', gap:'6px', flexWrap:'wrap' }}>
                                  {sitePhotos.map((url,i) => <div key={i} onClick={() => setBatchPhoto({ url, label:`Site · ${batch.company_name}` })} style={{ width:'72px', height:'72px', borderRadius:'8px', background:`url(${url}) center/cover`, cursor:'pointer', border:'1px solid #e5e7eb' }} />)}
                                </div>
                              </div>
                            )}
                          </div>
                        )}
                        {batch.notes && <div style={{ background:'#f9fafb', borderRadius:'8px', padding:'8px 10px', fontSize:'12px', color:'#374151', marginBottom:'8px' }}>📝 {batch.notes}</div>}
                        <div style={{ fontSize:'11px', color:'#9ca3af' }}>Updated: {new Date(batch.updated_at).toLocaleString('en-IN')}{batch.contact_email && <span style={{ marginLeft:'8px', color:'#166534' }}>📧 Approval email → {batch.contact_email}</span>}</div>
                      </div>
                    </div>
                  )
                })}
          ...
          {items.length > 0 && (
            <>
              {(csrBatches.length > 0 || miyawakis.length > 0) && (
                    ...
                    <div key={item.id} style={{ background:'white', border:'1px solid #e5e7eb', borderRadius:'12px', overflow:'hidden' }}>
                      <div style={{ padding:'8px 10px', background:'#f9fafb', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center', gap:'6px', flexWrap:'wrap' }}>
                        <div>
                          ...
                          <div style={{ fontSize:'11px', color:'#6B7280' }}>👷 {item.users?.name||'—'} · 📍 {item.trees?.sites?.name||'—'}{donor && <span style={{ color:'#2C5F2D', marginLeft:'6px' }}>· 🌿 {donor.name}</span>}</div>
                        </div>
                        <div style={{ display:'flex', gap:'5px' }}>
                          <button onClick={() => handleDecision(item.id, true)} disabled={processing} style={{ padding:'4px 12px', background:'#2C5F2D', color:'white', border:'none', borderRadius:'7px', fontSize:'12px', fontWeight:600, cursor:'pointer' }}>✓ Approve</button>
                          <button onClick={() => handleDecision(item.id, false)} disabled={processing} style={{ padding:'4px 12px', background:'transparent', color:'#dc2626', border:'1px solid #fecaca', borderRadius:'7px', fontSize:'12px', fontWeight:600, cursor:'pointer' }}>✕ Reject</button>
                        </div>
                      </div>
                      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr' }}>
                        <div style={{ borderRight:'1px solid #f3f4f6' }}>
                          <div style={{ padding:'5px 8px 3px', fontSize:'10px', fontWeight:600, color:'#6B7280', textTransform:'uppercase' }}>Before</div>
                          <div onClick={() => item.before_photo_url && setPhotoPopup({ url:item.before_photo_url, label:'Before · '+(item.trees?.species||''), treeId:item.trees?.tree_id||'', itemId:item.id, health:null, lat:captLat, lng:captLng })}
                            ...
                            {item.before_photo_url && <div style={{ position:'absolute', bottom:0, left:0, right:0, background:'rgba(0,0,0,0.5)', color:'white', fontSize:'9px', padding:'3px 7px', display:'flex', justifyContent:'space-between' }}><span>Before</span><span>🔍</span></div>}
                          </div>
                        </div>
                        <div>
                          <div style={{ padding:'5px 8px 3px', fontSize:'10px', fontWeight:600, color:'#6B7280', textTransform:'uppercase' }}>After</div>
                          <div onClick={() => afterUrl && setPhotoPopup({ url:afterUrl, label:'After · '+(item.trees?.species||''), treeId:item.trees?.tree_id||'', itemId:item.id, health:item.ai_health_score, lat:captLat, lng:captLng })}
                            ...
                            {!afterUrl && '🌳'}
                            {afterUrl && <>{item.ai_health_score!==null && <div style={{ position:'absolute', top:5, right:5, background:'rgba(0,0,0,0.65)', color:'white', fontSize:'10px', padding:'2px 7px', borderRadius:5, fontWeight:600 }}>{item.ai_health_score}%</div>}<div style={{ position:'absolute', bottom:0, left:0, right:0, background:'rgba(0,0,0,0.5)', color:'white', fontSize:'9px', padding:'3px 7px', display:'flex', justifyContent:'space-between' }}><span>After</span><span>🔍</span></div></>}
                          </div>
                        </div>
                      </div>
                      <div style={{ padding:'8px 10px', borderTop:'1px solid #f3f4f6', display:'flex', flexDirection:'column', gap:'6px' }}>
                        ...
                        {hasGPS && dist && (
                          <div style={{ background:item.ai_gps_match===false?'#fee2e2':'#f0fdf4', border:`1px solid ${item.ai_gps_match===false?'#fecaca':'#86efac'}`, borderRadius:6, padding:'4px 8px', fontSize:'11px', color:item.ai_gps_match===false?'#dc2626':'#166534', fontWeight:500, display:'flex', justifyContent:'space-between' }}>
                        ...
                        {donor ? <div style={{ fontSize:'10px', color:'#166534', background:'#f0fdf4', padding:'3px 8px', borderRadius:6 }}>📧 Verified email → {donor.email}</div>
                               : <div style={{ fontSize:'10px', color:'#9ca3af', background:'#f9fafb', padding:'3px 8px', borderRadius:6 }}>⚠️ No donor linked</div>}
      ...
      {/* PHOTO POPUPS */}
      {batchPhoto && (
        <div onClick={() => setBatchPhoto(null)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.92)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:'12px', padding:'1rem' }}>
          <div style={{ color:'rgba(255,255,255,0.5)', fontSize:'11px', textTransform:'uppercase' }}>Tap anywhere to close</div>
          ...
          <div style={{ background:'rgba(255,255,255,0.1)', borderRadius:'12px', padding:'10px 16px', color:'white', fontSize:'12px' }}>{batchPhoto.label}</div>
        </div>
      )}
      {photoPopup && (
        <div onClick={() => setPhotoPopup(null)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.92)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:'12px', padding:'1rem' }}>
          <div style={{ color:'rgba(255,255,255,0.5)', fontSize:'11px', textTransform:'uppercase' }}>Tap anywhere to close</div>
          <div style={{ position:'relative' }} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{ background:'rgba(255,255,255,0.1)', borderRadius:'12px', padding:'12px 20px', color:'white', fontSize:'12px', textAlign:'center', width:'min(90vw,480px)' }} onClick={e=>e.stopPropagation()}>
            ...
            {photoPopup.health!==null && <div style={{ marginBottom:'8px', fontWeight:600, color:'#97BC62' }}>Health: {photoPopup.health}/100</div>}
            ...
            <div style={{ display:'flex', gap:'8px', justifyContent:'center' }}>
              <button onClick={() => handleDecision(photoPopup.itemId, true)} disabled={processing} style={{ padding:'8px 24px', background:'#2C5F2D', color:'white', border:'none', borderRadius:'8px', fontSize:'13px', fontWeight:600, cursor:'pointer' }}>✓ Approve</button>
              <button onClick={() => handleDecision(photoPopup.itemId, false)} disabled={processing} style={{ padding:'8px 24px', background:'transparent', color:'#fca5a5', border:'1px solid #fca5a5', borderRadius:'8px', fontSize:'13px', fontWeight:600, cursor:'pointer' }}>✕ Reject</button>
            </div>
          </div>
        </div>
      )}
