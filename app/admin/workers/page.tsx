...
export default function AdminWorkers() {
  ...
  async function addWorker(e: React.FormEvent) {
    ...
    const { data: authData, error: authError } = await supabase.auth.admin
      ? { data: null, error: { message: 'Use Supabase dashboard to create workers' } }
    ...
    <div>
      ...
      {showForm && (
        <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '1.25rem', marginBottom: '1.5rem' }}>
          <h3 style={{ fontSize: '14px', fontWeight: 600, color: '#1A1A1A', marginBottom: '1rem' }}>Add new field worker</h3>
          <form onSubmit={addWorker}>
            <div className="form-grid">
              {[
                { label: 'Full name', key: 'name', type: 'text', ph: 'Raju Kumar' },
                { label: 'Email', key: 'email', type: 'email', ph: 'raju@gmail.com' },
        ...
        <>
          ...
          <div className="desk-table" style={{ background: 'white', borderRadius: '12px', border: '1px solid #e5e7eb', overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {workers.length === 0 ? (
                  <tr><td colSpan={5} style={{ padding: '2rem', textAlign: 'center', color: '#9ca3af', fontSize: '14px' }}>No workers yet — add your first worker above</td></tr>
                ) : workers.map(w => (
                  <tr key={w.id} style={{ borderTop: '1px solid #f3f4f6' }}>
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#1A3C34', color: '#97BC62', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 600, flexShrink: 0 }}>
                          {w.name.split(' ').map(n => n[0]).join('').substring(0,2).toUpperCase()}
                        </div>
                        <span style={{ fontSize: '14px', fontWeight: 500, color: '#1A1A1A' }}>{w.name}</span>
                      </div>
                    </td>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <span style={{ background: w.is_active ? '#dcfce7' : '#f3f4f6', color: w.is_active ? '#166534' : '#6B7280', fontSize: '12px', padding: '3px 8px', borderRadius: '6px', fontWeight: 500 }}>
                        {w.is_active ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td style={{ padding: '12px 16px' }}>
                      <button onClick={() => toggleActive(w)} style={{ fontSize: '12px', padding: '5px 12px', background: 'transparent', border: '1px solid #e5e7eb', borderRadius: '6px', cursor: 'pointer', color: '#374151' }}>
            ...
            {workers.map(w => (
              <div key={w.id} style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '1rem', marginBottom: '0.75rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#1A3C34', color: '#97BC62', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 600 }}>
                      {w.name.split(' ').map(n => n[0]).join('').substring(0,2).toUpperCase()}
                    </div>
                    <div>
                  ...
                  <span style={{ background: w.is_active ? '#dcfce7' : '#f3f4f6', color: w.is_active ? '#166534' : '#6B7280', fontSize: '11px', padding: '3px 8px', borderRadius: '6px', fontWeight: 500 }}>
                    {w.is_active ? 'Active' : 'Inactive'}
                  </span>
                </div>
                <div style={{ fontSize: '13px', color: '#6B7280', marginBottom: '10px' }}>📞 {w.phone || 'No phone'}</div>
                <button onClick={() => toggleActive(w)} style={{ fontSize: '13px', padding: '6px 14px', background: 'transparent', border: '1px solid #e5e7eb', borderRadius: '6px', cursor: 'pointer', color: '#374151' }}>
                  {w.is_active ? 'Deactivate' : 'Activate'}
                </button>
              </div>
            ))}
            {workers.length === 0 && (
              <div style={{ textAlign: 'center', color: '#9ca3af', fontSize: '14px', padding: '2rem' }}>No workers yet</div>
            )}
          </div>
        </>
      )}
