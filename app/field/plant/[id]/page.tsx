...
type Step = 'before' | 'after' | 'notes' | 'submitting' | 'done'
...
export default function FieldPlant() {
  ...
  useEffect(() => { loadTask() }, [assignmentId])
  ...
  function captureGPSManually() {
    ...
    navigator.geolocation.getCurrentPosition(
      pos => {
        setGps({ lat: pos.coords.latitude, lng: pos.coords.longitude, source: 'manual' })
        setGpsLoading(false)
      },
      err => {
  ...
  async function handlePhotoChange(e: React.ChangeEvent<HTMLInputElement>, type: 'before' | 'after') {
    ...
    if (type === 'before') {
      ...
      if (exifGps) {
        setGps({ ...exifGps, source: 'exif' })
        setGpsError('')
      } else {
        setGps(null)
        setGpsError('No GPS in photo — use manual button below')
      }
      setGpsLoading(false)
      setStep('notes')
    }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const { error: beforeErr } = await supabase.storage
        .from(bucket).upload(beforePath, beforePhoto, { contentType: 'image/jpeg', upsert: true })
      ...
      const { error: afterErr } = await supabase.storage
        .from(bucket).upload(afterPath, afterPhoto, { contentType: 'image/jpeg', upsert: true })
      ...
      await supabase.from('trees').update({ qr_code_url: qrCodeUrl }).eq('tree_id', treeId)
  ...
  if (step === 'before') return (
    <Screen title="Step 1 of 3" subtitle="Take BEFORE photo" treeId={treeId} site={task?.sites?.name}>
      <div style={{ flex:1, display:'flex', flexDirection:'column', gap:'1rem', padding:'1.25rem' }}>
        <div style={{ background:'#fef3c7', borderRadius:'12px', padding:'1rem', fontSize:'14px', color:'#92400e' }}>
          📸 Take a photo of the <strong>site before planting</strong> — show the empty ground.
        </div>
        ...
        {beforePreview ? (
          <div style={{ position:'relative', borderRadius:'16px', overflow:'hidden', flex:1, maxHeight:'40vh' }}>
            ...
            <button onClick={() => { setBeforePhoto(null); setBeforePreview('') }}
          ...
          <div onClick={() => beforeRef.current?.click()}
            style={{ flex:1, minHeight:'200px', background:'#f9fafb', border:'2px dashed #d1d5db', borderRadius:'16px', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', cursor:'pointer', gap:'0.75rem' }}>
        ...
        <input ref={beforeRef} type="file" accept="image/*" capture="environment"
          onChange={e => handlePhotoChange(e, 'before')} style={{ display:'none' }} />

        {beforePreview && (
          <button onClick={() => setStep('after')}
    ...
    <Screen title="Step 2 of 3" subtitle="Plant tree + take AFTER photo" treeId={treeId} site={task?.sites?.name}>
      <div style={{ flex:1, display:'flex', flexDirection:'column', gap:'1rem', padding:'1.25rem' }}>
        <div style={{ background:'#dcfce7', borderRadius:'12px', padding:'1rem', fontSize:'14px', color:'#166534' }}>
          🌱 Plant the tree first. Then take a photo — <strong>GPS will be auto-extracted from the photo.</strong>
          {' '}Make sure your phone camera has <strong>Location ON</strong>.
        </div>

        {afterPreview ? (
          <div style={{ position:'relative', borderRadius:'16px', overflow:'hidden', flex:1, maxHeight:'40vh' }}>
            <img src={afterPreview} alt="After" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
            <button onClick={() => { setAfterPhoto(null); setAfterPreview(''); setGps(null); setGpsError('') }}
              style={{ position:'absolute', top:'10px', right:'10px', background:'rgba(0,0,0,0.6)', border:'none', borderRadius:'8px', color:'white', padding:'6px 12px', fontSize:'13px', cursor:'pointer' }}>
              Retake
            </button>
          </div>
          ...
          <div onClick={() => afterRef.current?.click()}
            style={{ flex:1, minHeight:'200px', background:'#f0fdf4', border:'2px dashed #86efac', borderRadius:'16px', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', cursor:'pointer', gap:'0.75rem' }}>
            <span style={{ fontSize:'3rem' }}>🌱</span>
            <span style={{ fontSize:'15px', fontWeight:600, color:'#374151' }}>Tap to take photo</span>
            <span style={{ fontSize:'13px', color:'#9ca3af' }}>After planting · GPS auto-captured</span>
          </div>
        )}

        <input ref={afterRef} type="file" accept="image/*" capture="environment"
          onChange={e => handlePhotoChange(e, 'after')} style={{ display:'none' }} />
  ...
  if (step === 'notes') return (
    <Screen title="Step 3 of 3" subtitle="Health check + submit" treeId={treeId} site={task?.sites?.name}>
      <div style={{ flex:1, display:'flex', flexDirection:'column', gap:'1rem', padding:'1.25rem', overflowY:'auto' }}>
        ...
        {/* GPS status */}
        {gpsLoading ? (
          <div style={{ background:'#dbeafe', borderRadius:'12px', padding:'0.875rem 1rem', display:'flex', alignItems:'center', gap:'8px', fontSize:'13px', color:'#1e40af' }}>
            <span>⏳</span> Extracting GPS from photo...
          </div>
        ) : gps ? (
          <div style={{ background:'#dcfce7', borderRadius:'12px', padding:'0.875rem 1rem', display:'flex', alignItems:'center', gap:'8px', fontSize:'13px', color:'#166534' }}>
            <span>{gps.source === 'exif' ? '📷' : '📍'}</span>
            <div>
              <div style={{ fontWeight:600 }}>
                {gps.source === 'exif' ? '✅ GPS from photo EXIF' : '✅ GPS captured manually'}
              </div>
              <div style={{ fontFamily:'monospace', fontSize:'12px' }}>{gps.lat.toFixed(6)}° N, {gps.lng.toFixed(6)}° E</div>
            </div>
          </div>
        ) : (
          <div style={{ background:'#f9fafb', borderRadius:'12px', padding:'0.875rem 1rem' }}>
            <div style={{ fontSize:'13px', color:'#6B7280', marginBottom:'8px' }}>
              {gpsError || '⚠️ GPS not captured yet'}
            </div>
            <button onClick={captureGPSManually} disabled={gpsLoading}
              ...
              {gpsLoading ? '⏳ Getting GPS...' : '📍 Capture GPS Manually'}
            </button>
          </div>
        )}
        ...
        {/* Notes */}
        <div>
          <div style={{ fontSize:'14px', fontWeight:500, color:'#374151', marginBottom:'8px' }}>Notes (optional)</div>
          <textarea value={notes} onChange={e => setNotes(e.target.value)}
            placeholder="Any additional notes about this tree..."
            rows={3}
            style={{ width:'100%', padding:'0.875rem', border:'1.5px solid #e5e7eb', borderRadius:'12px', fontSize:'15px', outline:'none', resize:'none', fontFamily:'inherit' }} />
        </div>
        ...
        <button onClick={handleSubmit}
          style={{ width:'100%', padding:'1.125rem', background:'#1A3C34', color:'white', border:'none', borderRadius:'14px', fontSize:'16px', fontWeight:700, cursor:'pointer', minHeight:'60px', letterSpacing:'0.02em' }}>
          🌱 Submit Tree Report
        </button>
      </div>
    </Screen>
  ...
  if (step === 'done') return (
    <div style={{ minHeight:'100dvh', background:'#1A3C34', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'2rem', gap:'1rem', textAlign:'center' }}>
      ...
      <div style={{ background:'white', borderRadius:'16px', padding:'1.25rem', width:'100%', maxWidth:'280px', textAlign:'center' }}>
        ...
        <a href={`https://ecotrees.org/tree/${treeId}`} target="_blank" rel="noopener noreferrer"
