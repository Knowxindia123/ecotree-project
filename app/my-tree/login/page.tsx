  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (!donorData) {
      ...
      if (!userData || (userData.role !== 'DONOR' && userData.role !== 'ADMIN')) {
        ...
        setError('No donor account found for this email.')
        setLoading(false)
        return
      }
    }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ fontSize: '14px', color: '#6B7280', marginBottom: '1.5rem' }}>
          Sign in to track your trees and impact
        </p>
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div>
            ...
            <input
              ...
              required
              style={{ width: '100%', padding: '0.875rem 1rem', border: '1.5px solid #e5e7eb', borderRadius: '12px', fontSize: '15px', outline: 'none', fontFamily: 'inherit' }}
            />
          </div>
          ...
          <button
            ...
            style={{
              ...
              background: loading ? '#9ca3af' : 'linear-gradient(135deg, #2C5F2D, #40916C)',
              color: 'white', border: 'none', borderRadius: '12px',
              fontSize: '15px', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer',
              marginTop: '0.5rem', fontFamily: 'inherit', minHeight: '52px',
            }}
          >
            {loading ? 'Signing in...' : 'View my trees →'}
          </button>
        </form>

        <p style={{ textAlign: 'center', fontSize: '13px', color: '#9ca3af', marginTop: '1.5rem' }}>
          Don&apos;t have an account?{' '}
          <a href="/donate" style={{ color: '#2C5F2D', fontWeight: 600, textDecoration: 'none' }}>
      ...
      <p style={{ marginTop: '1.5rem', fontSize: '12px', color: 'rgba(255,255,255,0.3)', textAlign: 'center' }}>
        EcoTree Impact Foundation · Every impact verified
      </p>
