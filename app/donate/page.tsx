...
const TIERS = [
  { id:'community_100', tier:'100', icon:'🌿', name:'Community Contributor', price:100, co2:'~5kg', tag:'❤️ Most Accessible', tagColor:'#52B788', desc:'Join our community forest. Certificate in your name.', what:['Community forest certificate','Impact dashboard access','Project participation invites','Community tree tracking'], img:'https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?w=600&q=80', species:null, occasionIds:[], dashboard:'/community-dashboard', badge:'COMMUNITY', badgeColor:'#52B788' },
  { id:'community_250', tier:'250', icon:'🌱', name:'Community Supporter', price:250, co2:'~5kg', tag:'🌿 Great Value', tagColor:'#2D6A4F', desc:'Support our community forest with greater impact.', what:['Community forest certificate','Impact dashboard access','Priority project invites','Community tree tracking'], img:'https://images.unsplash.com/photo-1416879595882-3373a0480b5b?w=600&q=80', species:null, occasionIds:['birthday','anniversary','festival','baby'], dashboard:'/community-dashboard', badge:'COMMUNITY', badgeColor:'#52B788' },
  { id:'joint_500', tier:'500', icon:'🤝', name:'Joint Tree Donor', price:500, co2:'~11kg', tag:'👥 Share a Tree', tagColor:'#F59E0B', desc:'Pool with 1 stranger — together you plant 1 tree.', what:['Individual tree certificate','Shared tree dashboard','GPS location on map','Before & after photos','Species preference'], img:'https://images.unsplash.com/photo-1466692476868-aef1dfb1e735?w=600&q=80', species:['Neem','Peepal','Mango','Jamun','Any'], occasionIds:['birthday','anniversary','festival','baby'], dashboard:'/my-tree', badge:'JOINT', badgeColor:'#F59E0B' },
  { id:'individual_1000', tier:'1000', icon:'🌳', name:'Individual Tree', price:1000, co2:'~22kg', tag:'⭐ Most Popular', tagColor:'#1B4332', desc:'Your own tree. Full dashboard. GPS tracked for 3 years.', what:['Individual tree certificate','Personal tree dashboard','GPS location on map','Before & after photos','AI health score','80G tax receipt','Guaranteed species'], img:'https://images.unsplash.com/photo-1448375240586-882707db888b?w=600&q=80', species:['Neem','Peepal','Mango','Jamun','Guava','Rain Tree','Banyan','Gulmohar'], occasionIds:['birthday','anniversary','memory','festival','baby','corporate','custom'], dashboard:'/my-tree', badge:'INDIVIDUAL', badgeColor:'#1B4332' },
  { id:'miyawaki_5000', tier:'5000', icon:'🏙️', name:'Miyawaki Forest', price:5000, co2:'~200kg', tag:'🔥 Premium Impact', tagColor:'#7C3AED', desc:'30+ native species. Dense urban forest. 10x faster growth.', what:['Forest impact certificate','Miyawaki forest dashboard','GPS forest location','Species diversity report','BRSR-compatible report','80G tax receipt','Donor wall recognition'], img:'https://images.unsplash.com/photo-1425913397330-cf8af2ff40a1?w=600&q=80', species:null, occasionIds:['corporate','custom'], dashboard:'/miyawaki-dashboard', badge:'FOREST', badgeColor:'#7C3AED' },
]
...
async function sendEmail(type: string, donor: Record<string, any>) {
  try {
    await fetch('/api/send-email', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ type, donor }) })
...
export default function DonatePage() {
  ...
  : (existing.total_trees || 0) + 1,
      ...
      // ── JOINT ₹500 ──
        ...
        for (let qi = 0; qi < qty; qi++) {
          ...
          // Find oldest OPEN pool where this donor is NOT already a member (FIFO)
          ...
          if (targetPool) {
            ...
            if (newFilled >= targetPool.slots_total) {
              // ── POOL COMPLETE: create ONE shared tree ──
              ...
              const { data: newTree } = await supabase.from('trees').insert({
                tree_id: treeId, tree_type: 'Joint Tree', species: species || 'Neem',
                status: 'PENDING', donor_id: firstMember?.donor_id || donorId,
              ...
              // Get all members → increment total_trees + send matched email to both
              ...
              for (const m of members || []) {
                ...
                if (d?.email) {
                  await sendEmail('welcome', {
                    name: d.name, email: d.email, tree_id: treeId, species: species || 'Neem',
                    password: '123456', tier: '500', dashboard: '/my-tree', joint: true, matched: true,
                    partner: (members||[]).filter((mm:any) => { const dd = Array.isArray(mm.donors)?mm.donors[0]:mm.donors as any; return dd?.email !== d.email }).map((mm:any) => { const dd = Array.isArray(mm.donors)?mm.donors[0]:mm.donors as any; return dd?.name })[0] || 'your co-donor',
                  })
                }
              }
            ...
            // ── CREATE new pool (Donor 1 or already in all open pools) ──
  ...
  return (
    <main className="dn">
      <div className="dn-topbar">
        <div className="dn-c">
          ...
          <div className="dn-tb-r2">
            <div className="dn-tb-trust"><span>🌱 10,000+ trees</span><span className="dn-dt">·</span><span>🎯 91% survival</span><span className="dn-dt">·</span><span>🤖 AI-verified</span><span className="dn-dt">·</span><span>🧾 80G approved</span></div>
      ...
      <div className="dn-split-bg"><div className="dn-c dn-split">
          ...
          {mode === 'plant' ? (<>
              ...
              {TIERS.map(t => (
                <div key={t.id} className={`dn-tcard${tier.id===t.id?' sel':''}`} onClick={()=>pickTier(t)}>
                  ...
                  <div className="dn-tbody">
                    ...
                    <ul className="dn-twhat">{t.what.slice(0,3).map(w=><li key={w}>✓ {w}</li>)}</ul>
                  </div>
                </div>
              ))}
            </div>
            {tier.species && (<div className="dn-species">
              <label className="dn-sl">{tier.id==='individual_1000'?'Choose species *':'Species preference'}{tier.id==='joint_500'&&<span className="dn-opt"> (subject to availability)</span>}</label>
              <select value={species} onChange={e=>setSpecies(e.target.value)} className={`dn-sel${errors.species?' err':''}`}>
                <option value="">— Select species —</option>
                {tier.species.map(s=><option key={s} value={s}>{s}</option>)}
              </select>
              <Err msg={errors.species}/>
            </div>)}
            {tier.id==='miyawaki_5000'&&<div className="dn-species-note">🌿 30+ native species mixed forest — EcoTree selects for maximum biodiversity</div>}
          </>) : (<>
            ...
            <div className="dn-ogrid">
              {OCCASIONS.map(o=>(
                <div key={o.id} className={`dn-ocard${occ.id===o.id?' sel':''}`} onClick={()=>setOcc(o)}>
                  <div className="dn-oi">{o.icon}</div><div className="dn-ol">{o.label}</div><div className="dn-op">₹{o.price}</div>
            ...
            <div className="dn-gfields">
              ...
              <div className="dn-frow">
                <div className="dn-field"><label>Recipient name *</label><input type="text" placeholder="Who is this gift for?" value={form.recipientName} onChange={e=>sf('recipientName',e.target.value)} className={errors.recipientName?'err':''}/><Err msg={errors.recipientName}/></div>
                <div className="dn-field"><label>Recipient email *</label><input type="email" placeholder="Their email for gift certificate" value={form.recipientEmail} onChange={e=>sf('recipientEmail',e.target.value)} className={errors.recipientEmail?'err':''}/><Err msg={errors.recipientEmail}/></div>
              </div>
        ...
        <div className="dn-right" ref={formRef}><div className="dn-rcard">
          <div className="dn-sum">
            <div className="dn-sum-t">Your order</div>
            <div className="dn-sum-row">
              <span style={{display:'flex',alignItems:'center',gap:'8px'}}><span className="dn-sum-badge" style={{background:tier.badgeColor}}>{tier.badge}</span>{mode==='gift'?`${occ.icon} ${occ.label} Gift`:`${tier.icon} ${tier.name}`}</span>
              <span className="dn-sum-p">₹{total.toLocaleString('en-IN')}</span>
            </div>
            <div className="dn-sum-meta">
              {tier.id==='community_100'||tier.id==='community_250'?<><span>🌿 Community forest</span><span>📜 Certificate</span><span>📊 Dashboard</span></>
              :tier.id==='joint_500'?<><span>🤝 Shared tree</span><span>📜 Certificate</span><span>📍 GPS</span><span>🧾 80G</span></>
              :tier.id==='miyawaki_5000'?<><span>🏙️ 30+ species</span><span>📊 Forest dashboard</span><span>🧾 80G</span></>
          ...
          <div className="dn-form">
            <div className="dn-frow">
              <div className="dn-field"><label>Full name *</label><input type="text" placeholder="Your full name" value={form.name} onChange={e=>sf('name',e.target.value)} className={errors.name?'err':''}/><Err msg={errors.name}/></div>
              <div className="dn-field"><label>Email address *</label><input type="email" placeholder="your@email.com" value={form.email} onChange={e=>sf('email',e.target.value)} className={errors.email?'err':''}/><Err msg={errors.email}/></div>
            </div>
            <div className="dn-frow">
              <div className="dn-field"><label>Phone number *</label><input type="tel" placeholder="98860 94611" value={form.phone} onChange={e=>sf('phone',e.target.value)} className={errors.phone?'err':''}/><Err msg={errors.phone}/></div>
              <div className="dn-field"><label>City / address <span className="dn-opt">(optional)</span></label><input type="text" placeholder="Bangalore" value={form.address} onChange={e=>sf('address',e.target.value)}/></div>
            </div>
            <div className="dn-frow">
              <div className="dn-field"><label>Birthday <span className="dn-opt">(for anniversary wishes)</span></label><input type="date" value={form.birthday} onChange={e=>sf('birthday',e.target.value)}/></div>
          ...
          <button className="dn-paybtn" onClick={handlePay} disabled={loading}>{loading?<><span className="dn-spin"/> Saving...</>:payBtnLabel()}</button>
      ...
      <section className="dn-sec dn-sec--how"><div className="dn-c">
        ...
        <div className="dn-how">
          {[{icon:'🌱',n:'01',t:'We Plant',d:'Our field team plants your tree at a verified Bangalore site within 7 days.'},{icon:'📍',n:'02',t:'GPS Tagged',d:'A unique GPS coordinate and QR code is assigned to your specific tree.'},{icon:'🤖',n:'03',t:'AI Verified',d:'Claude AI verifies species, location, timestamp and health before it reaches you.'},{icon:'📊',n:'04',t:'Live Dashboard',d:'Track your tree with monthly photos and AI health scores for 3 full years.'}].map((s,i)=>(
            <React.Fragment key={s.n}><div className="dn-hi"><div className="dn-hico">{s.icon}</div><div className="dn-hn">{s.n}</div><div className="dn-ht">{s.t}</div><div className="dn-hd">{s.d}</div></div>{i<3&&<div className="dn-harr">→</div>}</React.Fragment>
          ))}
        </div>
      </div></section>

      <section className="dn-sec dn-sec--white"><div className="dn-c">
        ...
        <div className="dn-gal">
          {[{img:'https://images.unsplash.com/photo-1509391366360-2e959784a276?w=500&q=80',l:'🌱 Day 1 — Sapling planted',s:'GPS-tagged, QR code attached'},{img:'https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?w=500&q=80',l:'🌿 Month 6 — Growing strong',s:'AI health score: 94%'},{img:'https://images.unsplash.com/photo-1448375240586-882707db888b?w=500&q=80',l:'🌳 Year 3 — Full canopy',s:'22kg CO₂ offset/year'}].map(g=>(
            <div className="dn-gi" key={g.l}><img src={g.img} alt={g.l} className="dn-gimg" loading="lazy"/><div className="dn-gcap"><div className="dn-gl">{g.l}</div><div className="dn-gs">{g.s}</div></div></div>
          ))}
        </div>
      </div></section>

      <div className="dn-tr"><div className="dn-c dn-tr__in">
      ...
      <section className="dn-sec dn-sec--offwhite"><div className="dn-c dn-c--n"><div className="dn-lbl">FAQ</div><h2 className="dn-h2">Quick answers</h2><FAQ/></div></section>

      <div className="dn-bcta"><div className="dn-c dn-bcta__in">
      ...
      <style>{`
        ...
        .dn-c{max-width:1200px;margin:0 auto;padding:0 1.5rem;}.dn-c--n{max-width:740px;}.dn-sec{padding:3rem 0;}.dn-sec--white{background:#fff;}.dn-sec--offwhite{background:var(--ow);}.dn-sec--how{background:#fff;border-top:1px solid var(--md);border-bottom:1px solid var(--md);}
        ...
        .dn-h2{font-size:clamp(1.5rem,3vw,2.2rem);font-weight:800;line-height:1.18;color:var(--td);margin:0 0 1.5rem;}.dn-h2 em{color:var(--ga);font-style:normal;}
        .dn-opt{font-weight:400;color:var(--tm);font-size:0.78rem;}.dn-dt{color:rgba(151,188,98,0.5);}
        .dn-topbar{position:sticky;top:80px;z-index:50;background:var(--gd);border-bottom:1px solid rgba(151,188,98,0.25);box-shadow:0 2px 12px rgba(0,0,0,0.25);}.dn-topbar .dn-c{padding-top:0.6rem;padding-bottom:0;}
        ...
        .dn-tb-headline{display:flex;flex-direction:column;gap:0.1rem;}.dn-tb-h{font-size:0.92rem;font-weight:700;color:#fff;line-height:1.3;}.dn-tb-h em{color:var(--gl);font-style:normal;}.dn-tb-sub{font-size:0.72rem;color:rgba(255,255,255,0.55);}
        .dn-tb-acts{display:flex;gap:0.5rem;flex-shrink:0;}.dn-sbtn{display:inline-flex;align-items:center;gap:0.35rem;font-size:0.75rem;font-weight:700;padding:0.38rem 0.9rem;border-radius:999px;cursor:pointer;border:none;font-family:inherit;transition:all 0.2s;white-space:nowrap;}.dn-sbtn--wa{background:#25D366;color:#fff;}.dn-sbtn--cp{background:rgba(255,255,255,0.12);color:#fff;border:1px solid rgba(255,255,255,0.2);}
        .dn-tb-r2{display:flex;align-items:center;justify-content:space-between;gap:1rem;}.dn-tb-trust{display:flex;align-items:center;gap:0.6rem;flex-wrap:wrap;font-size:0.75rem;font-weight:600;color:rgba(255,255,255,0.65);}
        .dn-tb-tabs{display:flex;gap:0;}.dn-tab{padding:0.7rem 1.4rem;border:none;border-bottom:3px solid transparent;cursor:pointer;font-size:0.92rem;font-weight:700;color:rgba(255,255,255,0.6);background:transparent;transition:all 0.2s;font-family:inherit;white-space:nowrap;}.dn-tab.active{color:#fff;border-bottom-color:var(--ga);}
        .dn-split-bg{background:var(--ow);padding:2rem 0;}.dn-split{display:grid;grid-template-columns:1.15fr 0.85fr;gap:2rem;align-items:start;}@media(max-width:900px){.dn-split{grid-template-columns:1fr;}}
        ...
        .dn-tgrid{display:flex;flex-direction:column;gap:0.75rem;margin-bottom:1rem;}.dn-tcard{display:grid;grid-template-columns:110px 1fr;border:2px solid var(--md);border-radius:var(--r);overflow:hidden;cursor:pointer;transition:all 0.2s;background:#fff;}.dn-tcard:hover,.dn-tcard.sel{border-color:var(--ga);box-shadow:0 4px 16px rgba(82,183,136,0.2);}@media(max-width:480px){.dn-tcard{grid-template-columns:85px 1fr;}}
        .dn-timg-w{position:relative;overflow:hidden;}.dn-timg{width:100%;height:100%;object-fit:cover;min-height:90px;display:block;transition:transform 0.3s;}.dn-tcard:hover .dn-timg{transform:scale(1.05);}
        .dn-ttag{position:absolute;top:0.4rem;left:0.4rem;font-size:0.58rem;font-weight:700;color:#fff;padding:0.15rem 0.4rem;border-radius:999px;}.dn-tbadge{position:absolute;bottom:0.4rem;left:0.4rem;font-size:0.56rem;font-weight:800;color:#fff;padding:0.1rem 0.4rem;border-radius:3px;letter-spacing:0.06em;}.dn-tchk{position:absolute;top:0.4rem;right:0.4rem;width:20px;height:20px;background:var(--ga);color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:0.7rem;font-weight:700;}
        .dn-tbody{padding:0.8rem 0.9rem;}.dn-ttop{display:flex;justify-content:space-between;align-items:flex-start;gap:0.4rem;margin-bottom:0.2rem;}.dn-tname{font-size:0.88rem;font-weight:700;color:var(--td);}.dn-tprice{font-size:1rem;font-weight:900;color:var(--gd);white-space:nowrap;}.dn-tdesc{font-size:0.75rem;color:var(--tm);margin-bottom:0.35rem;line-height:1.4;}.dn-twhat{list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:0.15rem;}.dn-twhat li{font-size:0.72rem;color:var(--gm);font-weight:500;}
        .dn-species{background:#fff;border:1.5px solid var(--md);border-radius:0.75rem;padding:0.9rem 1rem;margin-bottom:0.75rem;}.dn-sl{display:block;font-size:0.84rem;font-weight:700;color:var(--tb);margin-bottom:0.5rem;}.dn-sel{width:100%;padding:0.7rem 0.9rem;border:1.5px solid var(--md);border-radius:0.65rem;font-size:0.95rem;color:var(--td);background:#fff;outline:none;font-family:inherit;cursor:pointer;}.dn-sel.err,.dn-field input.err,.dn-field textarea.err{border-color:#dc2626 !important;}.dn-sel:focus{border-color:var(--ga);}
        ...
        .dn-ogrid{display:grid;grid-template-columns:repeat(4,1fr);gap:0.6rem;}@media(max-width:480px){.dn-ogrid{grid-template-columns:repeat(2,1fr);}}
        .dn-ocard{position:relative;border:2px solid var(--md);border-radius:0.75rem;padding:0.85rem 0.4rem;cursor:pointer;text-align:center;transition:all 0.2s;background:#fff;}.dn-ocard:hover,.dn-ocard.sel{border-color:var(--ga);background:var(--mt);}.dn-oi{font-size:1.4rem;margin-bottom:0.2rem;}.dn-ol{font-size:0.74rem;font-weight:700;color:var(--td);margin-bottom:0.15rem;}.dn-op{font-size:0.78rem;font-weight:800;color:var(--gd);}.dn-och{position:absolute;top:0.3rem;right:0.3rem;width:15px;height:15px;background:var(--ga);color:#fff;border-radius:50%;font-size:0.58rem;display:flex;align-items:center;justify-content:center;font-weight:700;}
        .dn-gfields{display:flex;flex-direction:column;gap:0.85rem;}
        .dn-rcard{background:#fff;border:1.5px solid var(--md);border-radius:var(--r);padding:1.6rem;position:sticky;top:160px;}.dn-sum-t{font-size:0.7rem;font-weight:700;text-transform:uppercase;letter-spacing:0.12em;color:var(--tm);margin-bottom:0.7rem;}.dn-sum-row{display:flex;justify-content:space-between;align-items:center;font-size:0.95rem;font-weight:700;color:var(--td);margin-bottom:0.45rem;gap:0.5rem;}.dn-sum-badge{font-size:0.58rem;font-weight:800;color:#fff;padding:0.15rem 0.5rem;border-radius:3px;letter-spacing:0.06em;flex-shrink:0;}.dn-sum-p{font-size:1.1rem;font-weight:900;color:var(--gd);flex-shrink:0;}.dn-sum-meta{display:flex;flex-wrap:wrap;gap:0.3rem;}.dn-sum-meta span{font-size:0.7rem;background:var(--mt);color:var(--gd);padding:0.18rem 0.5rem;border-radius:999px;font-weight:600;}
        .dn-div{height:1px;background:var(--md);margin:1rem 0;}.dn-form{display:flex;flex-direction:column;gap:0.75rem;margin-bottom:1rem;}.dn-frow{display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;}@media(max-width:540px){.dn-frow{grid-template-columns:1fr;}}
        .dn-field{display:flex;flex-direction:column;gap:0.3rem;}.dn-field label{font-size:0.84rem;font-weight:700;color:var(--tb);}.dn-field input,.dn-field textarea{width:100%;padding:0.72rem 0.9rem;border:1.5px solid var(--md);border-radius:0.65rem;font-size:0.95rem;color:var(--td);background:#fff;outline:none;transition:border-color 0.2s,box-shadow 0.2s;font-family:inherit;box-sizing:border-box;}.dn-field input:focus,.dn-field textarea:focus{border-color:var(--ga);box-shadow:0 0 0 3px rgba(82,183,136,0.1);}.dn-field input::placeholder,.dn-field textarea::placeholder{color:#AABFB4;font-size:0.85rem;}.dn-field textarea{resize:vertical;min-height:70px;}
        ...
        .dn-paybtn{width:100%;padding:1rem;background:#2C5F2D;color:#fff;border:none;border-radius:0.75rem;font-size:0.97rem;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:0.5rem;transition:all 0.2s;font-family:inherit;box-shadow:0 4px 20px rgba(44,95,45,0.3);margin-bottom:0.65rem;}.dn-paybtn:hover:not(:disabled){filter:brightness(1.08);transform:translateY(-1px);}.dn-paybtn:disabled{opacity:0.7;cursor:not-allowed;transform:none;}
        .dn-spin{display:inline-block;width:15px;height:15px;border:2px solid rgba(255,255,255,0.3);border-top-color:#fff;border-radius:50%;animation:sp 0.6s linear infinite;}@keyframes sp{to{transform:rotate(360deg);}}
        .dn-paymet{text-align:center;font-size:0.74rem;color:var(--tm);margin-bottom:0.35rem;display:flex;justify-content:center;gap:0.35rem;}.dn-paytr{display:flex;justify-content:space-between;font-size:0.73rem;color:var(--tm);}
        .dn-how{display:flex;align-items:flex-start;gap:0;margin-top:2rem;}@media(max-width:640px){.dn-how{flex-wrap:wrap;gap:1.25rem;}.dn-harr{display:none;}}
        .dn-hi{flex:1;min-width:120px;text-align:center;padding:1.25rem 0.65rem;background:#fff;border-radius:var(--r);border:1px solid var(--md);margin:0 0.18rem;box-shadow:0 1px 8px rgba(27,67,50,0.05);}.dn-hico{font-size:2.2rem;margin-bottom:0.5rem;}.dn-hn{font-size:0.62rem;font-weight:700;color:var(--ga);letter-spacing:0.12em;text-transform:uppercase;margin-bottom:0.3rem;}.dn-ht{font-size:0.93rem;font-weight:800;color:var(--gd);margin-bottom:0.3rem;}.dn-hd{font-size:0.76rem;color:var(--tm);line-height:1.5;}.dn-harr{font-size:1.2rem;color:var(--ga);padding-top:3rem;flex-shrink:0;opacity:0.5;}
        .dn-gal{display:grid;grid-template-columns:repeat(3,1fr);gap:1.25rem;}@media(max-width:640px){.dn-gal{grid-template-columns:1fr;}}.dn-gi{border-radius:var(--r);overflow:hidden;border:1px solid var(--md);}.dn-gimg{width:100%;aspect-ratio:4/3;object-fit:cover;display:block;transition:transform 0.3s;}.dn-gi:hover .dn-gimg{transform:scale(1.03);}.dn-gcap{padding:0.8rem 1rem;background:#fff;}.dn-gl{font-size:0.88rem;font-weight:700;color:var(--gd);margin-bottom:0.15rem;}.dn-gs{font-size:0.74rem;color:var(--tm);}
        .dn-tr{background:var(--gd);padding:1.5rem 0;}.dn-tr__in{display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;}@media(max-width:700px){.dn-tr__in{grid-template-columns:repeat(2,1fr);}}.dn-ti{display:flex;align-items:flex-start;gap:0.65rem;}.dn-tii{font-size:1.3rem;flex-shrink:0;}.dn-tt{font-size:0.85rem;font-weight:700;color:#fff;margin-bottom:0.1rem;}.dn-ts{font-size:0.7rem;color:rgba(255,255,255,0.55);line-height:1.4;}
        .dn-bcta{background:var(--mt);padding:1rem 0;border-top:1px solid var(--md);}.dn-bcta__in{display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;}.dn-bcta__in>span{font-size:0.88rem;font-weight:600;color:var(--gd);}.dn-bcta__btns{display:flex;gap:0.6rem;}
        .dn-btn{display:inline-flex;align-items:center;justify-content:center;gap:0.4rem;font-size:0.88rem;font-weight:600;padding:0.6rem 1.3rem;border-radius:999px;text-decoration:none;cursor:pointer;border:none;transition:all 0.2s;font-family:inherit;white-space:nowrap;}.dn-btn--p{background:#2C5F2D;color:#fff;}.dn-btn--wa{background:#25D366;color:#fff;}
        .dn-tinline-qty{display:flex;align-items:center;gap:4px;flex-shrink:0;}.dn-qb{width:26px;height:26px;border-radius:50%;border:1.5px solid var(--md);background:#fff;font-size:1rem;cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--gd);font-family:inherit;flex-shrink:0;line-height:1;}.dn-qn{min-width:24px;text-align:center;font-size:0.9rem;font-weight:800;color:var(--gd);}
...
function FAQ() {
  ...
  return (
    <div className="fq">
      {qs.map((f,i)=>(<div key={i} className="fqi"><button className={`fqq${o===i?' op':''}`} onClick={()=>setO(o===i?null:i)}><span>{f.q}</span><span className="fqc">{o===i?'−':'+'}</span></button>{o===i&&<div className="fqa">{f.a}</div>}</div>))}
      <style>{`.fq{display:flex;flex-direction:column;gap:0.6rem;margin-top:1.5rem;}.fqi{border:1px solid #B7E4C7;border-radius:0.6rem;overflow:hidden;}.fqq{width:100%;display:flex;justify-content:space-between;align-items:center;gap:1rem;padding:1rem 1.1rem;font-size:0.92rem;font-weight:600;color:#1B4332;background:#F8FAF8;border:none;cursor:pointer;text-align:left;font-family:inherit;transition:background 0.15s;}.fqq:hover,.fqq.op{background:#D8F3DC;}.fqc{font-size:1.2rem;color:#52B788;flex-shrink:0;}.fqa{padding:0.9rem 1.1rem;font-size:0.9rem;line-height:1.7;color:#2D3B36;background:#fff;border-top:1px solid #B7E4C7;}`}</style>
    </div>
  )
}
